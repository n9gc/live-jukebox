 ...
 */
export function generateZodInferredType({ aliasName, zodImportValue, zodConstName, typeMetadata, }) {
    let typeReference;
    switch (typeMetadata) {
        case "none":
            typeReference = f.createTypeReferenceNode(f.createQualifiedName(f.createIdentifier(zodImportValue), f.createIdentifier("infer")), [f.createTypeQueryNode(f.createIdentifier(zodConstName))]);
            break;
        case "promise":
            // For Promise types (not functions), use Promise<z.output<>>
            // because z.infer<z.promise<T>> returns T instead of Promise<T> in Zod v4
            const outputType = f.createTypeReferenceNode(f.createQualifiedName(f.createIdentifier(zodImportValue), f.createIdentifier("output")), [f.createTypeQueryNode(f.createIdentifier(zodConstName))]);
        ...
        case "promiseReturningFunction":
            ...
            typeReference = f.createTypeReferenceNode(f.createQualifiedName(f.createIdentifier(zodImportValue), f.createIdentifier("output")), [f.createTypeQueryNode(f.createIdentifier(zodConstName))]);
            break;
    }
    return f.createTypeAliasDeclaration([f.createModifier(ts.SyntaxKind.ExportKeyword)], f.createIdentifier(aliasName), undefined, typeReference);
}
