 ...
 * @returns Tags list
 */
export function getJSDocTags(nodeType, sourceFile) {
 ...
 * @param isNullable
 */
export function jsDocTagToZodProperties(jsDocTags, customJSDocFormats, isOptional, isPartial, isRequired, isNullable) {
    const zodProperties = [];
    if (jsDocTags.minimum !== undefined) {
        zodProperties.push({
            identifier: "min",
            expressions: withErrorMessage(jsDocTags.minimum.value < 0
                ? f.createPrefixUnaryExpression(ts.SyntaxKind.MinusToken, f.createNumericLiteral(Math.abs(jsDocTags.minimum.value)))
                : f.createNumericLiteral(jsDocTags.minimum.value), jsDocTags.minimum.errorMessage),
        });
    }
    if (jsDocTags.maximum !== undefined) {
        zodProperties.push({
            ...
            expressions: withErrorMessage(jsDocTags.maximum.value < 0
                ? f.createPrefixUnaryExpression(ts.SyntaxKind.MinusToken, f.createNumericLiteral(Math.abs(jsDocTags.maximum.value)))
                : f.createNumericLiteral(jsDocTags.maximum.value), jsDocTags.maximum.errorMessage),
        });
    }
    let hasFormat = false;
    if (jsDocTags.format &&
            ...
            expressions: jsDocTags.default === true
                ...
                : jsDocTags.default === false
                    ...
                    : typeof jsDocTags.default === "number"
                        ? jsDocTags.default < 0
                            ? [
                                f.createPrefixUnaryExpression(ts.SyntaxKind.MinusToken, f.createNumericLiteral(Math.abs(jsDocTags.default))),
                            ]
...
function createZodRegexProperty(regex, errorMessage) {
    return {
        identifier: "regex",
        expressions: withErrorMessage(f.createRegularExpressionLiteral(`/${regex}/`), errorMessage),
    };
}
...
function createArrayLiteralExpression(arr) {
    const elements = arr.map((item) => {
        if (typeof item === "string")
            return f.createStringLiteral(item);
        if (typeof item === "number")
            return f.createNumericLiteral(item);
...
function createObjectLiteralExpression(obj) {
    const properties = Object.entries(obj).map(([key, value]) => {
        ...
        if (typeof value === "string")
            return f.createPropertyAssignment(propertyName, f.createStringLiteral(value));
        if (typeof value === "number")
            return f.createPropertyAssignment(propertyName, f.createNumericLiteral(value));
        if (typeof value === "boolean")
            return f.createPropertyAssignment(propertyName, value ? f.createTrue() : f.createFalse());
        if (value === null)
            return f.createPropertyAssignment(propertyName, f.createNull());
        if (Array.isArray(value))
            return f.createPropertyAssignment(propertyName, createArrayLiteralExpression(value));
        if (typeof value === "object")
            return f.createPropertyAssignment(propertyName, createObjectLiteralExpression(value));
        return f.createPropertyAssignment(propertyName, f.createStringLiteral(String(value)));
    });
