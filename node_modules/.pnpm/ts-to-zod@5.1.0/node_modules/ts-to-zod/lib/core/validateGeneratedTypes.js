...
import { getImportPath, areImportPathsEqualIgnoringExtension, } from "../utils/getImportPath.js";
/**
 * Use typescript compiler to validate the generated zod schemas.
 */
export function validateGeneratedTypes({ sourceTypes, zodSchemas, integrationTests, skipParseJSDoc, extraFiles = [], }) {
    // Shared configuration
    const compilerOptions = {
        target: ts.ScriptTarget.ES2016,
        esModuleInterop: true,
    };
    const sourceFile = ts.createSourceFile("index.ts", skipParseJSDoc
        ? sourceTypes.sourceText
        : resolveDefaultProperties(sourceTypes.sourceText), ts.ScriptTarget.Latest);
    // Extracting imports that should be handled as any
    const importsToHandleAsAny = new Set();
    const extractImportIdentifiers = (node) => {
        if (ts.isImportDeclaration(node) &&
            ...
            !extraFiles.find((file) => {
                ...
                return areImportPathsEqualIgnoringExtension(getImportPath(sourceTypes.relativePath, file.relativePath), node.moduleSpecifier.text);
            })) {
    ...
    const fixedSourceForValidation = fixOptionalAny(sourceFile, importsToHandleAsAny);
    ...
    const env = createVirtualTypeScriptEnvironment(system, [sourceTypes, zodSchemas, integrationTests].map(getPath), ts, compilerOptions);
    // Get the diagnostic
    const errors = [];
