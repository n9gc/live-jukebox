...
import { getReferencedTypeNames, isTypeNode, } from "../utils/traverseTypes.js";
import { getImportIdentifiers, createImportNode, getSingleImportIdentifierForNode, } from "../utils/importHandling.js";
...
import { generateZodSchemaVariableStatement, generateZodSchemaVariableStatementForImport, } from "./generateZodSchema.js";
...
const DEFAULT_GET_SCHEMA = (id) => camelCase(id) + "Schema";
/**
 ...
 */
export function generate({ sourceText, nameFilter = () => true, jsDocTagFilter = () => true, getSchemaName = DEFAULT_GET_SCHEMA, keepComments = false, skipParseJSDoc = false, customJSDocFormatTypes = {}, inputOutputMappings = [], }) {
    ...
    const typeNameMapBuilder = (node) => {
        ...
        if (ts.isImportDeclaration(node) && node.importClause) {
            ...
            const eligibleMapping = inputOutputMappings.find((io) => {
                if (!ts.isStringLiteral(node.moduleSpecifier))
                    throw new Error("node.moduleSpecifier must be a StringLiteral");
                return areImportPathsEqualIgnoringExtension(io.input, node.moduleSpecifier.text);
            });
            if (eligibleMapping) {
                const schemaMethod = eligibleMapping.getSchemaName || DEFAULT_GET_SCHEMA;
                identifiers
                    .filter(({ name }) => nameFilter(name))
                    .forEach(({ name }) => importedZodNamesAvailable.set(name, schemaMethod(name)));
                const zodImportNode = createImportNode(identifiers.map(({ name, original }) => {
                    return {
    ...
    candidateTypesToBeExtracted.forEach((typeRef) => {
        ...
        if (node) {
            ...
            if (typeRef.partOfQualifiedName) {
                ...
                const importIdentifier = getSingleImportIdentifierForNode(node, typeRef.name);
                if (!importIdentifier)
                    return;
                if (identifiers) {
...
${Array.from(zodSchemasWithMissingDependencies).join("\n")}`);
    ...
    const zodImportToOutput = zodImportNodes
        ...
        if (!ts.isStringLiteral(node.moduleSpecifier))
            throw new Error("node.moduleSpecifier must be a StringLiteral");
        return createImportNode(getImportIdentifiers(node).filter(({ name }) => importedZodSchemas.has(name)), node.moduleSpecifier.text);
    });
    const originalImportsToOutput = Array.from(importsToKeep.keys()).map((node) => {
        if (!ts.isStringLiteral(node.moduleSpecifier))
            throw new Error("node.moduleSpecifier must be a StringLiteral");
        return createImportNode(importsToKeep.get(node), node.moduleSpecifier.text);
    });
...
${zodImportToOutput.length
        ...
        .join("\n\n")}
`;
    const testCases = generateIntegrationTests(Array.from(statements.values())
        .filter(isExported)
        .filter((statement) => {
        ...
        const isPromiseFunction = schema?.typeMetadata === "promiseReturningFunction";
 ...
 * @returns
 */
const isExported = (i) => i.value.modifiers?.find((mod) => mod.kind === ts.SyntaxKind.ExportKeyword);
