 ...
 */
export async function createConfig(configPath, tsToZodConfigFileName) {
    ...
    while (true) {
        ...
        const output = await ask("Where do you want to save the generated zod schemas?", { initialValue: input.replace(/\.ts(x)?$/, ".zod.ts$1") });
 ...
 *
 * @type {${isDev ? 'import("./src/config")' : 'import("ts-to-zod")'}.TsToZodConfig}
 */
export default `;
    if (configs.length > 0) {
        await writeFile(configPath, header + JSON.stringify(configs.length === 1 ? configs[0] : configs), "utf-8");
        return true;
    }
    return false;
}
 ...
 */
async function ask(message, opt) {
