import { Command, Flags, Errors, Args } from "@oclif/core";
...
import { getSchemaNameSchema, nameFilterSchema, tsToZodConfigSchema, } from "./config.zod.js";
import { generate } from "./core/generate.js";
import { createConfig } from "./createConfig.js";
import { areImportPathsEqualIgnoringExtension, getImportPath, } from "./utils/getImportPath.js";
...
if (configPath.endsWith(".js")) {
    ...
    const newTsToZodConfigFileName = tsToZodConfigFileName.replace(/.js$/, `.${format}`);
    await rename(configPath, newConfigPath);
    prompt.log.success(`${tsToZodConfigFileName} has been renamed to ${newTsToZodConfigFileName}`);
    configPath = newConfigPath;
    tsToZodConfigFileName = newTsToZodConfigFileName;
    if (format === "mjs") {
        const oldConfig = await readFile(configPath, "utf-8");
        await writeFile(configPath, oldConfig.replace(/module\.exports =/, "export default"));
...
try {
    ...
    if (hasConfigFile) {
        const rawConfig = await import(slash(relative(import.meta.dirname, configPath)));
    ...
    if (e instanceof Error) {
        Errors.error(`"${tsToZodConfigFileName}" invalid:
    ${e.message}
...
class TsToZod extends Command {
    ...
    async run() {
        ...
        try {
            const rawTsConfig = await readFile(join(process.cwd(), "tsconfig.json"), "utf-8");
            ...
            explicitFileExtImports = ["node16", "nodenext"].includes(tsConfig.compilerOptions.moduleResolution);
        }
        catch { }
        ...
        if (Array.isArray(fileConfig)) {
            ...
            tasks.add(fileConfig.map((config) => ({
                title: `Generate "${config.name}"`,
                task: async (_, task) => task.newListr(this.generate({
                    args,
                    fileConfig: config,
                    flags,
                    ioMappings,
        ...
        if (flags.watch) {
            ...
            chokidar.watch(inputs).on("change", async (path) => {
                ...
                const config = Array.isArray(fileConfig)
                    ...
                    : fileConfig;
                const tasks = new Listr(this.generate({
                    args,
                    fileConfig: config,
                    flags,
                    ioMappings,
     ...
     */
    generate({ args, fileConfig, flags, ioMappings, explicitFileExtImports, }) {
...
See more help with --help`);
        ...
        const generateZodSchemasTask = {
            ...
            task: async (ctx) => {
                ...
                const relativeIOMappings = ioMappings.map((io) => {
                    const relativeInput = getImportPath(inputPath, io.input) + importPathExt;
                    const relativeOutput = getImportPath(outputPath, io.output) + importPathExt;
                    return {
                        input: relativeInput,
                ...
                if (ctx.generatedOutput.hasCircularDependencies && !output) {
                    throw new Error("--output= must also be provided when input files have some circular dependencies");
                }
            },
        };
        const validationTask = {
            ...
            task: async (ctx) => {
                if (!ctx.generateOptions || !ctx.generatedOutput) {
                    throw new Error("Validation can only be perform after the generation");
                }
                const { skipParseJSDoc } = ctx.generateOptions;
                const { transformedSourceText, getIntegrationTestFile, getZodSchemasFile, } = ctx.generatedOutput;
                const extraFiles = [];
                for (const io of ioMappings) {
                    if (getImportPath(inputPath, io.input) !== "/") {
                        ...
                        try {
                            ...
                            throw new Error(`File "${io.output}" not found: maybe it hasn't been generated yet?`);
                        }
                    }
                }
                ...
                const generationErrors = await worker.validateGeneratedTypesInWorker({
                    ...
                    integrationTests: {
                        sourceText: getIntegrationTestFile(getImportPath("./source.integration.ts", input) + importPathExt, getImportPath("./source.integration.ts", outputForValidation) +
                            importPathExt),
                        relativePath: "./source.integration.ts",
                    },
                    zodSchemas: {
                        sourceText: getZodSchemasFile(getImportPath(outputForValidation, input) + importPathExt),
        ...
        const writingFilesTask = {
            title: "Write files",
            task: async (ctx, task) => {
                if (!ctx.generateOptions || !ctx.generatedOutput) {
                    throw new Error("Writing files can only be perform after the generation");
                }
                ...
                const zodSchemasFile = getZodSchemasFile(getImportPath(outputPath, inputPath) + importPathExt);
                if (inferredTypes) {
                    subtasks.add({
                        title: `Write ${relative(process.cwd(), inferredTypes)}`,
                        task: async () => {
                            const zodInferredTypesFile = getInferredTypes(getImportPath(inferredTypes, outputPath) + importPathExt);
                            await writeFile(inferredTypes, hasExtensions(inferredTypes, javascriptExtensions)
                ...
                subtasks.add({
                    ...
                    task: async () => {
                        if (output && hasExtensions(output, javascriptExtensions)) {
                            await writeFile(outputPath, ts.transpileModule(zodSchemasFile, {
                                compilerOptions: {
                                    target: ts.ScriptTarget.Latest,
                                    module: ts.ModuleKind.ESNext,
                                    newLine: ts.NewLineKind.LineFeed,
