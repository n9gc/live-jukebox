...
function fingersCrossed(sink, options = {}) {
	...
	if (bufferLevel != null) {
		try {
			compareLogLevel("trace", bufferLevel);
		} catch (error) {
			throw new TypeError(`Invalid bufferLevel: ${JSON.stringify(bufferLevel)}. ${error instanceof Error ? error.message : String(error)}`);
		}
		if (compareLogLevel(bufferLevel, triggerLevel) >= 0) throw new RangeError(`bufferLevel (${JSON.stringify(bufferLevel)}) must be lower than triggerLevel (${JSON.stringify(triggerLevel)}).`);
	}
