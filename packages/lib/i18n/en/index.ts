...
const en: Base = {
	...
	jukebox: {
		'auto-picker': {
			pickFailed: 'Auto pick failed, because {result: CommonPickerException|enums}.',
		...
		'song-list': {
			...
			endTooLate: 'For {title: string|quote}, marking it as end too late.',
			endTooEarly: 'For {title: string|quote}, marking it as end too early. '
				+ 'Dropping {songs: string[]|length} song{{s}}: {songs: string[]|quotes}.',
			cancelWhilePlaying: 'Cancelling from {pickerDisplay: string|rawLog} failed, cannot cancel while playing.',
			noCancelable: 'no song can be canceled by {pickerDisplay: string|rawLog}.',
			canceled: '{pickerDisplay: string|rawLog} canceled song {title: string|quote} successly',
		},
	},
	player: {},
	reader: {},
